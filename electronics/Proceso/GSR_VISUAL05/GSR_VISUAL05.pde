...
//buffer arrays
int[] readingBuffer = new int[sWidth];
...
void setup() {
  ...
  noStroke();
  background(0);
  mySerial = new Serial(this, Serial.list()[0], 19200);
...
void draw() {
  ...
  x = x+lineRange;
  if (x > sWidth)   //if it goes offscreen, loop back to the left
    x = 0;
  lastRead = currentRead;
  fadeCounter++;

  point(frameCount%width, pY);
  if (frameCount%width!=0)
    line(ppx, ppy, frameCount%width, pY);
  else background(0);
  println("Py: "+pY);
  ppx=frameCount%width;
  ppy=pY;
}
...
void eraseTop() {
  noStroke();
  fill(0, 0, 0, 255);
  rect(0, 0, width, sHeight-absHigh-1);
}
void eraseText() {
  noStroke();
  fill(0, 0, 0, 255);
...
void serialEvent(Serial myPort) {
  ...
  while (mySerial.available() > 0) {
    ...
    if (inByte != ',') { 
      ...
      pY=int(inByte);
      //pY = int(map(pY, 600, 1023, 100, 200));
    } else {
